...
func (c Config) buildDSN() string {
	return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=utf8mb4&parseTime=true&loc=Asia%%2FShanghai&timeout=5s&readTimeout=120s&writeTimeout=120s",
...
func New(cfg Config) (db *gorm.DB, err error) {
	...
	if err != nil {
		return db, fmt.Errorf("error when init mysql, err: %v", err)
	}
